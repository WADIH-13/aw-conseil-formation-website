...
async function getFormatorBySlug(slug: string): Promise<FormatorData | null> {
  try {
    console.log('🔍 Detail page: Looking for slug:', slug);
    ...
    console.log('📖 Query result - Error:', error?.message || null, 'Data:', data ? { id: data.id, name: data.name, slug: data.slug, hasIntentions: !!data.intentions } : null);
    ...
    // ...existing code for the main return (en dehors du if !formator)
  } catch (error) {
...
export default async function FormatorPage({ params }: PageProps) {
  ...
  const formator = await getFormatorBySlug(slug);
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center space-y-4">
            ...
            Retour à la liste
                    ...
                    <section className="max-w-5xl mx-auto px-6 pt-20 pb-10 md:pt-28 md:pb-16">
                      <div className="flex flex-col md:flex-row gap-12 md:gap-20 items-center md:items-start">
                        ...
                        <div className="flex-1 flex flex-col gap-7 md:gap-10">
                          ...
                          <p className="text-2xl md:text-3xl text-aw-red font-semibold mt-1">
                            {formator.title}
                          </p>
                    ...
                    <section className="max-w-5xl mx-auto px-6 py-20 border-t border-gray-100 flex flex-col gap-12">
                      ...
                      <div className="flex flex-col md:flex-row gap-8 md:gap-10">
                        <div className="flex-1 bg-white rounded-2xl border-2 border-aw-red/20 shadow-lg p-8 flex flex-col gap-4 items-start hover:shadow-2xl transition-all duration-200">
                          ...
                          <p className="text-gray-700 leading-relaxed text-lg md:text-xl" style={{lineHeight: '2'}}>{formator.builds}</p>
                        </div>
                        <div className="flex-1 bg-white rounded-2xl border-2 border-aw-red/20 shadow-lg p-8 flex flex-col gap-4 items-start hover:shadow-2xl transition-all duration-200">
                          <p className="text-xs md:text-sm font-semibold text-aw-red uppercase tracking-wider mb-1">Ce qu'il / elle protège</p>
                          <p className="text-gray-700 leading-relaxed text-lg md:text-xl" style={{lineHeight: '2'}}>{formator.protects}</p>
                        </div>
                        <div className="flex-1 bg-white rounded-2xl border-2 border-aw-red/20 shadow-lg p-8 flex flex-col gap-4 items-start hover:shadow-2xl transition-all duration-200">
                          <p className="text-xs md:text-sm font-semibold text-aw-red uppercase tracking-wider mb-1">Ce qu'il / elle développe</p>
                          <p className="text-gray-700 leading-relaxed text-lg md:text-xl" style={{lineHeight: '2'}}>{formator.develops}</p>
                        </div>
                      </div>
                    </section>
                    ...
                    <section className="max-w-5xl mx-auto px-6 py-20 border-t border-gray-100 flex flex-col gap-10">
                      ...
                      <div className="flex flex-wrap gap-5 md:gap-8">
                        {formator.values && formator.values.map((value: string) => (
                          <button
                            key={value}
                    ...
                    <section className="max-w-5xl mx-auto px-6 py-20 border-t border-gray-100 flex flex-col gap-10">
                      ...
                      <div className="pt-4 md:pt-8">
                        <Link
                          ...
                          className="inline-flex items-center gap-3 text-aw-red font-semibold hover:gap-5 transition-all text-lg md:text-xl"
                        >
          ...
          <div className="flex-1 bg-white rounded-xl border-2 border-aw-red/30 shadow-sm p-6 flex flex-col gap-3 items-start hover:shadow-lg transition-all duration-200">
            <p className="text-xs md:text-sm font-semibold text-aw-red uppercase tracking-wider mb-1">Ce qu'il / elle développe</p>
            <p className="text-gray-700 leading-relaxed text-base md:text-lg" style={{lineHeight: '1.9'}}>{formator.develops}</p>
          </div>
        </div>
      </section>

      {/* Values */}
      <section className="max-w-4xl mx-auto px-6 py-16 md:py-20 border-t border-gray-200 flex flex-col gap-8">
        <h2 className="text-2xl md:text-3xl font-light text-gray-900 mb-6 md:mb-10">Ce qui guide son action</h2>
        <div className="flex flex-wrap gap-4 md:gap-6">
          {formator.values && formator.values.map((value: string) => (
            <button
              key={value}
              className="px-6 py-3 border-2 border-aw-red text-aw-red font-semibold rounded-full bg-white shadow-sm hover:bg-aw-red hover:text-white hover:scale-105 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-aw-red/50"
              type="button"
              tabIndex={0}
            >
              {value}
      ...
      {/* Closing */}
      <section className="max-w-3xl mx-auto px-6 py-16 md:py-20 border-t border-gray-200 flex flex-col gap-8">
        ...
        <div className="pt-4 md:pt-8">
          <Link
            href="/avancent-avec-aw"
            className="inline-flex items-center gap-2 text-aw-red font-medium hover:gap-3 transition-all"
          >
            <span>Découvrir d'autres trajectoires</span>
            <span>→</span>
          </Link>
