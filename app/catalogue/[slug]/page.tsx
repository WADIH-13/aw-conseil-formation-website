...
export default async function OfferDetailPage({
  ...
  const { data: offer, error } = await supabase
    .from("offers")
    .select("id, title, slug, summary, objectives, audience, modalities, formats, families(name)")
    .eq("slug", params.slug)
    .eq("is_active", true)
    .single();
  ...
  return (
    <div className="mx-auto max-w-4xl px-6 py-12">
      ...
      <div className="mt-8 flex flex-wrap gap-3">
        <Link
          href={`/contact?motif=programme&offre=${encodeURIComponent(offer.title)}`}
      ...
      <div className="mt-10 grid grid-cols-1 gap-6">
        ...
        {offer.audience && (
          <section className="rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm">
            <h2 className="text-lg font-semibold text-neutral-900">Public</h2>
            <pre className="mt-3 whitespace-pre-wrap text-neutral-700 leading-relaxed font-sans">
              {offer.audience}
            </pre>
          </section>
